...
'                Dim notFinishedPaths = paths.Where(Function(p)
'                                                       Return Not p.DeadEnded _
'                                                       And Not p.EndPointFound _
'                                                       And p.Path.Count < shortPathLength
'                                                   End Function).ToList()
...
'            'If the distance of this path is greater than the shortest path from start block to lastBlock, stop iterate further.
'            If newPath.Path.Count() - 1 > blocks(newX, newY).Distance Then
'                Return Nothing
'            End If
'            'Update the distance to lastBlock
'            blocks(newX, newY).Distance = newPath.Path.Count() - 1
...
'            Dim paths = GetPaths()
'            Dim validPaths = paths.Where(Function(p)
'                                             Return p.EndPointFound And p.Path.Count() = shortPathLength
'                                         End Function).ToList()
'            Return validPaths
'        End Function
'    End Class
...
'        Dim internalClass As New ShortPathInternal_AllPath(blocks, startBlock, endBlock)
